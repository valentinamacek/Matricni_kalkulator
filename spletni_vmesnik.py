...
OPERACIJE = ["transponiraj", "prirejenka", "inverz", "determinanta", "sled"]
...
def odstrani_matriko_post():
    ...
    if not izbrane:
        napake = {
            "izbor": "Izberi vsaj eno matriko , ki jo želiš izbrisati, ali pritisni prekliči"}
...
def operacija_ena(stanje_r, id_matrike, stopnja, skalar):
    # izracuna rezultate operacij glede na izbrano matriko
    # uporabljena je v vecih formsih, kaj spremeni je odvisno glede na to kaksne so vrednosti spremenljivk v url
    # ko izbiramo id_matrike, skalar ali stopnjo imajo vrednost n, potem pa se nastavijo na vrednost, ki je bila izbrana
    stanje = stanje_trenutnega_uporabnika()
    if id_matrike == 'n':
    ...
    if not isinstance(prirejenka, Matrika):
        napake.update(prirejenka)
    inverz = matrika.inverz()
    if not isinstance(inverz, Matrika):
        napake.update(inverz)
    determinanta = matrika.det()
    if not isinstance(determinanta, float):
        napake.update(determinanta)
    sled = matrika.sled()
    if not isinstance(sled, float):
        napake.update(sled)
    # stanje_r je primarno 0, pri kliku na gumb Shrani rezultat v seznam mojih matrik,
    # pa je njegova vrednost odvisna od tega, kateri rezultat zelimo shraniti
    if stanje_r == 1:
    ...
    if stopnja != 'p':
        ...
        if not isinstance(rezultat_potenciranja, Matrika):
            napake.update(rezultat_potenciranja)
        else:
            if stanje_r == 4:
                napaka = stanje.preveri_podatke_nove_matrike(
    ...
    if skalar != 's':
        ...
        if not isinstance(rezultat_mnozenjas, Matrika):
...
def operacija_dve(stanje_r, id_matrike1, id_matrike2):
    ...
    if not isinstance(vsota, Matrika):
        napake.update(vsota)
    produkt = matrika1 * matrika2
    if not isinstance(produkt, Matrika):
...
bottle.run(debug=True, host="localhost", port=8080, reloader=True)
